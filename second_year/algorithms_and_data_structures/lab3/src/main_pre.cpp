...
bool setUpVertex(GraphAdjRecordPointer graphAdj, unsigned firstVertex, unsigned secondVertex){
  ...
  while (currentAdjRecord->next != NULL){
    ...
    currentAdjRecord = currentAdjRecord->next;
    //cout << "went next" << endl;
    //cout << (currentAdjRecord == NULL) << endl;
  }
  //cout << "selected" << endl;
  currentAdjRecord->next = (GraphAdjRecordPointer)malloc(sizeof(GraphAdjRecord));
  currentAdjRecord = currentAdjRecord->next;
...
int main(int argc, char* argv[]){
  unsigned numberOfVertexes = 30;
  unsigned numberOfEdges = 50;
