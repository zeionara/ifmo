...
bool setUpVertex(GraphAdjRecordPointer graphAdj, unsigned firstVertex, unsigned secondVertex, unsigned weight){
...
bool appendConnectedVertex(GraphAdjRecordPointer graphAdj, unsigned firstVertex, unsigned secondVertex, unsigned weight){
  setUpVertex(graphAdj, firstVertex, secondVertex, weight);
  setUpVertex(graphAdj, secondVertex, firstVertex, weight);
  return true;
}
...
EdgePointer makeConnectedGraph(EdgePointer edges, unsigned numberOfVertexes, unsigned numberOfEdges, GraphAdjRecordPointer graphAdj, unsigned* weights, unsigned minWeight, unsigned maxWeight){
  srand(time(NULL));
  GraphAdjRecordPointer currentAdjRecord = graphAdj;
  ...
  for (int i = 1; i < numberOfVertexes; i++){
    ...
    appendConnectedVertex(graphAdj, i, i+1, weights[i]);
  }
  ...
  weights[numberOfVertexes] = (rand() % (maxWeight - minWeight)) + minWeight;
  appendConnectedVertex(graphAdj, numberOfVertexes, 1, weights[numberOfVertexes]);
  for (unsigned i = numberOfVertexes+1; i <= numberOfEdges; i++){
    ...
    weights[i] = (rand() % (maxWeight - minWeight)) + minWeight;
    appendConnectedVertex(graphAdj, firstVertex, secondVertex, weights[i]);
  }
...
int main(int argc, char* argv[]){
  ...
  //ofstream ate("log.txt");
  //cout << "OK" << endl;
  //cout.rdbuf(ate.rdbuf()); //redirect std::cout to out.txt!
  ...
  //while (numberOfEdges <= 10000000){
    edges = (EdgePointer)realloc(edges,(numberOfEdges+1)*sizeof(Edge));
    weights = (unsigned*)realloc(weights,(numberOfEdges+1)*sizeof(unsigned));
    graphAdj = (GraphAdjRecordPointer)realloc(graphAdj,(numberOfVertexes+1)*sizeof(GraphAdjRecord));
    //cout << "making graph" << endl;
    edges = makeConnectedGraph(edges, numberOfVertexes, numberOfEdges, graphAdj,weights, minWeight, maxWeight);
