...
if __name__ == "__main__":
    ...
    generator = Generator()
    #statistics = generator.generate_statistics(quantity_of_columns, quantity_of_numbers, low_bound, high_bound)
    #statistics = generator.generate_statistics_exp(quantity_of_columns, quantity_of_numbers, low_bound, high_bound, 1000)
    statistics = generator.generate_statistics_erl(quantity_of_columns, quantity_of_numbers, low_bound, high_bound, 1000, 4)
    ...
    worksheet.write(1,0,"Quantity")
    
    for i in range(len(statistics[0])):
        worksheet.write(0,i+1,"%i - %i" % (statistics[0][i][0],statistics[0][i][1]))
        worksheet.write(1,i+1,statistics[1][i])

    chart.add_series({
        'values': [worksheet.name] + [1,1] + [1, len(statistics[0])],
        'categories': [worksheet.name] + [0,1] + [0, len(statistics[0])],
        'name': "Random data",
    })
##
