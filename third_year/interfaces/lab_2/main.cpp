...
#define DECREASE_CONTRAST	2 //Decreases contrast
...
void decreaseContrast(HWND hwnd, HBITMAP hBitmap);
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdParam, int nCmdShow) {
  ...
  char imageStr[] = "Image";
	...
	AppendMenu(MainMenu, MF_STRING, DECREASE_CONTRAST, TEXT("Decrease contrast"));
...
LRESULT CALLBACK handleWindowEvents(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	case WM_COMMAND: {
		...
		if (LOWORD(wParam) == DECREASE_CONTRAST) {
			if (!isLoaded) {
				break;
			}
			decreaseContrast(hwnd, hBitmap);
		}
		break;
	}
	default: {
...
void decreaseContrast(HWND hwnd, HBITMAP hBitmap) {
    ...
    for (long i = 0; i < bmp.bmWidth * bmp.bmHeight * 3; i += 3) {
				...
				lpBits[i] = (WORD)(0.5 * (lpBits[i]  - 128) + 128);
				lpBits[i+1] = (WORD)(0.5 * (lpBits[i+1]  - 128) + 128);
				lpBits[i+2] = (WORD)(0.5 * (lpBits[i+2]  - 128) + 128);
