...
public:
	Edit();
	~Edit(void);
	bool Create(_In_ HINSTANCE hInstance,
...
static TCHAR* GetClipboardText(HWND hWnd);
static void toClipboard(TCHAR* chValue, int length_, HWND hWnd);
	...
	_In_ int       nCmdShow)
{
	...
	Edit edit;
	edit.Create(hInstance, hPrevInstance, lpCmdLine, nCmdShow);

	return 0;
}

bool Edit::Create(_In_ HINSTANCE hInstance,
	_In_opt_ HINSTANCE hPrevInstance,
	_In_ LPWSTR    lpCmdLine,
	_In_ int       nCmdShow) {
	UNREFERENCED_PARAMETER(hPrevInstance);
...
//
ATOM Edit::MyRegisterClass(HINSTANCE hInstance)
{
    WNDCLASSEXW wcex;
...
//
BOOL Edit::InitInstance(HINSTANCE hInstance, int nCmdShow)
{
...
TCHAR* Edit::GetClipboardText(HWND hWnd)
{
...
void Edit::toClipboard(TCHAR* chValue, int length_, HWND hWnd) {
...
LRESULT CALLBACK Edit::WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	RECT rect;
	LPTSTR lpstr = L"OK";
	int width;
